change.pt.pois.regMCMC.lambdasolo.not.zero <- function(n.mcmc,X,y.seed,opt.ht.strt,hshape1,hshape2,opt.ht.tune,lambda.shape,lambda.tune,lambda.rate,lambda.strt){
    ...
    x.star = as.vector(by(X[,3] > opt.ht.star,X[,1],sum))
 	  ...
 	  mh1.ht = sum(dpois(y.seed[x > 0,],lambda%*%W,log=TRUE)) + dgamma(opt.ht.star,hshape1,hshape2,log=TRUE)
	  mh2.ht = sum(dpois(y.seed[x > 0,],lambda%*%W,log=TRUE)) + dgamma(opt.ht,hshape1,hshape2,log=TRUE)
	...
	if(lambda.star > 0){
	  ...
	  mh1.lambda = sum(dpois(y.seed[x > 0,],lambda.star%*%W,log=TRUE))+dgamma(lambda.star,lambda.shape,lambda.rate,log=TRUE)
	  mh2.lambda = sum(dpois(y.seed[x > 0,],lambda%*%W,log=TRUE))+dgamma(lambda,lambda.shape,lambda.rate,log=TRUE)
