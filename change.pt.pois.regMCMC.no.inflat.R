change.pt.pois.regMCMC.no.inflat <- function(n.mcmc,X,y.seed,opt.ht.strt,hshape1,hshape2,beta,beta.mns,beta.tune,beta.var){
    ...
    ###
    if (TRUE){
    opt.ht.star = runifdisc(1,opt.ht-150,opt.ht+150)
    
    if(opt.ht.star>0 & opt.ht.star<=max(X[,3])){
    
    x.star = cbind(1,log(X1[,list(A=sum(V3>opt.ht.star),B=V1),by=V1][,A]))
    
 	  mh1.ht = sum(dpois(as.vector(y.seed[x.star[,2]>=0,]),as.vector(exp(x.star[x.star[,2]>=0,]%*%beta)%*%W),log=TRUE)) + dunifdisc(opt.ht.star,hshape1,hshape2) + dunifdisc(opt.ht,opt.ht-150,opt.ht+150)
	  mh2.ht = sum(dpois(as.vector(y.seed[x.star[,2]>=0,]),as.vector(exp(x[x.star[,2]>=0,]%*%beta)%*%W),log=TRUE)) + dunifdisc(opt.ht,hshape1,hshape2) + dunifdisc(opt.ht.star,opt.ht-150,opt.ht+150)
    ...
    if (TRUE){
	  ...
	  mh1.beta = sum(dpois(as.vector(y.seed[x[,2]>=0,]),as.vector(exp(x[x[,2]>=0,]%*%beta.star)%*%W),log=TRUE))+dmvnorm(beta.star,beta.mns,beta.var*diag(2),log=TRUE)
	  mh2.beta = sum(dpois(as.vector(y.seed[x[,2]>=0,]),as.vector(exp(x[x[,2]>=0,]%*%beta)%*%W),log=TRUE))+dmvnorm(beta,beta.mns,beta.var*diag(2),log=TRUE) #rep(beta.var,2)*diag(2)#solve(t(x[x[,2]>0,])%*%x[x[,2]>0,])
   ...
   if(mh.beta > runif(1)){
    ...
    list(ypred=ypred,msediffsave=msediffsave,p.value=p.value,opt.ht.save=opt.ht.save,beta.save=beta.save,accept.beta=accept.beta,accept.opt.ht=accept.opt.ht)
}